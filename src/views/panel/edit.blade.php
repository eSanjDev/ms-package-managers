...
@section('content')
                    ...
                    <div class="col-12 position-relative select-box mb-4">
                        <div class="form-floating">
                            <select name="uses_token" class="form-select select2">
                                <option @selected(old('uses_token',$manager->uses_token) == 1) value="1">Yes
                                </option>
                                <option @selected(old('uses_token',$manager->uses_token) == 0) value="0">No
                                </option>
                            </select>
    ...
    <div class="row">
        <div class="col-lg-12">
            @foreach(config('esanj.manager.extra_field') as $item)
                @include($item)
            @endforeach
        </div>
    </div>
